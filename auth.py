"""
EdWise Group — Student Authentication Module
File: student_auth.py
Usage: Import in Student Verification pages only.
"""
...
def _load_vendor_credentials(vendor_key: str) -> dict:
    """Load vendor-specific API credentials for Student Verification."""
    try:
        ...
        return {
            ...
            "base_api_url": block["base_api_url"],
        }
...
def render_login_page():
    ...
    module_label = "Dex Vendor Certification"
    module_icon = "🎓"
...
def render_logout_button(sidebar=True):
    ...
    with (st.sidebar if sidebar else st):
        if st.button("🔓 Sign Out", key="logout_btn_student", use_container_width=True):
            for key in ["logged_in", "current_vendor", "vendor_name", "vendor_creds", "token_info"]:
